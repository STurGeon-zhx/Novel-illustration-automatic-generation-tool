...
class TaskRepository:
    ...
    def create_task(self, chapter_text, image_count):
        task_id = uuid.uuid4().hex
        with closing(self._connect()) as conn:
            conn.execute(
                """
                INSERT INTO tasks (id, chapter_text, image_count, status)
                VALUES (?, ?, ?, ?)
                """,
                (task_id, chapter_text, image_count, "created"),
            )
            conn.commit()
        return task_id
    ...
    def replace_prompts(self, task_id, prompts):
        with closing(self._connect()) as conn:
